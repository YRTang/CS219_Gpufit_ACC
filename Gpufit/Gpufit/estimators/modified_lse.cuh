    ...
    std::size_t const user_info_size)
{
    ...
    if (weight)
    {
        gradient[point_index] = derivative[parameter_index] * deviation * weight[point_index];
    }
    else
    {
        gradient[point_index] = derivative[parameter_index] * deviation;

        if (parameter_index % 4 == 1 && deviation <= 0)
        {
            gradient[point_index] *= -1;
        }
